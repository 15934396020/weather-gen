...
The test dataset for the experiments consists of two categories: (a) programmatically generated synthetic scene images (e.g., a simplified cityscape with buildings, windows, roads, and sidewalks), which provide clean and reproducible structural inputs; and (b) real-world photographs of urban streets, natural landscapes, and architectural scenes, which are used to test the system's robustness with complex inputs. The synthetic images were generated using Python's PIL library, while the real-world photographs were sourced from open-license image collections.
...
It should be noted that this preliminary experiment uses text-to-image generation without structural control input. In the next phase, the full ControlNet pipeline will be deployed on a GPU server, enabling the system to generate weather-specific images that also respect the structural layout provided by the user's control image (e.g., Canny edge map). The combination of structural control and weather prompt engineering is expected to produce results with both accurate scene structure and convincing weather effects, which is the core objective of this project."""
